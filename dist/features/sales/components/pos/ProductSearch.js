...
function ProductSearch(_a) {
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, axios_1.default.get('/pos/search', { params: { q: query } })];
                    case 2:
                        data = (_c.sent()).data;
                        setResults((_b = data.data) !== null && _b !== void 0 ? _b : []);
                        return [3 /*break*/, 5];
                    case 3:
                        _a = _c.sent();
                        setResults([]);
    ...
    return (<div className="relative">
      <div className="relative">
        ...
        <input value={query} onChange={function (e) { return setQuery(e.target.value); }} placeholder="Buscar por nombre, SKU o código..." className="w-full pl-9 pr-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
      </div>
      {results.length > 0 && (<div className="absolute z-10 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-64 overflow-y-auto">
          {results.map(function (variant) { return (<button key={variant.id} onClick={function () { onSelect(variant); setQuery(''); setResults([]); }} className="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-50 text-left border-b last:border-0">
              <div>
